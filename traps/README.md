...
`user/call.c`가 컴파일되어 생성되는 `user/call.asm`을 보고 몇 가지의 질문에 답하는 실습이다.
...
A. `main()`에서 호출하는 `printf()`의 첫 번째 인자는 `f(8) + 1`이므로 12가 되고, 두 번째 인자는 13이다.
...
24 라인과 26 라인을 보면, 첫 번째 인자인 12는 `a1`에 들어가고, 두 번째 인자인 13은 `a2`에 들어가는 것을 알 수 있다.

Q. Where is the call to function `f` in the assembly code for main? Where is the call to `g`? (Hint: the compiler may inline functions.)

A. `make qemu`를 실행했을 때 출력되는 컴파일 명령어들 중에서 `call.c`를 컴파일하는 부분을 찾아보면 다음과 같다.
...
첫 번째 줄에서 `gcc`의 최적화 옵션인 `-O` 옵션을 확인할 수 있다. 이 옵션 때문에 `f()`가 인라인 함수로 최적화되어 들어가게 된다.

로컬에서 테스트해보면, `-O` 옵션을 빼고 돌리면 `f()`를 호출하는 과정이 추가되는 것을 확인할 수 있다.

Q. At what address is the function `printf` located?

A. `printf()`는 `0x630`에 위치한다.
...
A. `ra`는 return address를 저장하는 레지스터로, 함수 호출 후 돌아올 주소를 저장한다.
...
따라서 `printf()` 호출 직후에 `ra`에는 `0x38`이 저장된다.
...
A. 57616은 16진수로 `0xe110`이고, `i`는 리틀 엔디안에서 메모리에 `72 6c 64 00`으로 저장되기 때문에 문자열로 표현하면 `"rld"`가 된다. 따라서 리틀 엔디안 시스템인 RISC-V에서는 `"He110 World"`가 출력된다.

빅 엔디안에서는 `i`가 메모리에 `00 64 6c 72`로 저장된다. 따라서 `%s`로 출력했을 때 가장 먼저 널 문자를 만나기 때문에 `"He110 Wo"`가 출력된다. 57616의 경우 `%x`로 16진수를 출력하기 때문에 리틀 엔디안과 빅 엔디안에서 출력되는 값은 다르지 않다.
...
A. `printf()`의 두 번째 `%d`는 함수의 세 번째 인자를 저장하는 레지스터에서 값을 가져와서 정수 형식으로 출력하는데, 코드에서 세 번째 인자를 정해주지 않았기 때문에 원래 레지스터에 저장되어 있던 쓰레기값을 출력하게 된다. 따라서 정확하게 예측할 수 없다.
...
힌트의 지시사항대로, `riscv.h`에 frame pointer를 읽어오는 `r_fp()` 함수를 추가한다.
...
제공된 [lecture notes](https://pdos.csail.mit.edu/6.828/2020/lec/l-riscv-slides.pdf)에 따르면 return address는 frame pointer보다 한 칸 낮은 주소에 위치한다.
...
테스트 파일인 `alarmtest.c`가 컴파일되도록 `Makefile`에 추가한다.
...
`alarmtest.c`에서 사용되는 `sigalarm`과 `sigreturn` 시스템 콜을 구현해야 한다.

`syscall.h`에 시스템 콜 번호를 추가한다.
...
`alarmtest.c`에서 시스템 콜을 사용할 수 있도록 `user/user.h`와 `user/usys.pl`에 추가한다.
...
`sysproc.c`에 `sys_sigalarm()`과 `sys_sigreturn()`을 정의한다. `sys_sigreturn()`은 일단 비워둔다.
...
`usertrap()`에서 `which_dev`가 2인 경우(timer interrupt), `yield()` 대신 `p->tick_passed`를 1 증가시키고 `p->alarm_interval`과 같으면 `p->alarm_handler`로 점프하는 코드를 추가한다.
...
`sigreturn` 시스템 콜은 `sigalarm`의 handler function으로 점프한 이후 다시 원래 코드로 돌아오는 역할을 한다.
...
`sysproc.c`에 `sys_sigreturn()`을 구현한다. Backup trapframe을 다시 복구시킨다.
